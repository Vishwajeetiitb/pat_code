...
sumoCmd = [sumoBinary, "-c", "grid_5_5.sumocfg",
...
class rl_env(object):
    ...
    def step(self, action, idle, i):
        ...
        self.state, self.reward, action = self.check_step(
            curr_state, self.state, idle, action, i)
...
def run(env,run_id):
    global workbook,workbook1,workbook2
    # workbook = xlsxwriter.Workbook('./data/cr1/centre/'+'run'+str(run_id)+'.xlsx')
    np.random.seed(0)
    worksheet = workbook.add_worksheet('run'+str(run_id))
    worksheet1 = workbook1.add_worksheet('run'+str(run_id))
    worksheet2 = workbook2.add_worksheet('run'+str(run_id))
    ...
    idle_2d1 = np.zeros([num_steps, 25])
    idle_2d2 = np.zeros([num_steps, 25])
    while traci.simulation.getMinExpectedNumber() > 0:
        idle_2d[int(sumo_step)-1] = np.transpose(global_idl)
        idle_2d1[int(sumo_step)-1] = np.transpose(idle[0])
        idle_2d2[int(sumo_step)-1] = np.transpose(idle[1])
        # print(np.transpose(global_idl))
    ...
    previous_element = None
    index = 0
    for element in idle_2d1[:, node_id]:
        if element == 0 and previous_element is not None:
            peaks.append(previous_element)
            steps.append(index)
        previous_element = element
        index = index + 1
    plt.plot(steps, peaks)
    for col, data in enumerate(np.transpose(idle_2d1)):
        worksheet1.write_column(0, col+1, data)
    worksheet1.write_column(0, 0, range(num_steps))
    ...
    previous_element = None
    index = 0
    for element in idle_2d2[:, node_id]:
        if element == 0 and previous_element is not None:
            peaks.append(previous_element)
            steps.append(index)
        previous_element = element
        index = index + 1
    plt.plot(steps, peaks)
    for col, data in enumerate(np.transpose(idle_2d2)):
        worksheet2.write_column(0, col+1, data)
    worksheet2.write_column(0, 0, range(num_steps))

    global s
    message = 'q'
    s.send(message.encode('utf-8'))
    # print(idle_2d)
    # plt.plot(range(num_steps),idle_2d[:, 12])

    # plt.plot(ss, ga, "-r", linewidth=0.6, label="Global Average Idleness")
    # plt.plot(ss, gav, "-b", linewidth=4,
...
if __name__ == '__main__':
    ...
    workbook1 = xlsxwriter.Workbook(path+'runs1.xlsx')
    workbook2 = xlsxwriter.Workbook(path+'runs2.xlsx')
    ...
    print('workbook done')
    workbook1.close()
    print('workbook1 done')
    workbook2.close()
    print('workbook2 done')
#end of main

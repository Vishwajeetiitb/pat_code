...
for dead in deads:
	...
	print(avg)
	if check == 0:
		plt.plot(cars, avg, 'r-',label =str(dead)+" without intent")
	elif check == 1:
		plt.plot(cars, avg, 'g-',label =str(dead)+" without intent")
	else:
		plt.plot(cars, avg, 'b-',label =str(dead)+" without intent")
	check += 1
	plt.draw()
check = 0
for dead in deads:
	...
	print(avg)
	if check == 0:
		plt.plot(cars, avg, 'r--',label =str(dead)+" with intent")
	elif check == 1:
		plt.plot(cars, avg, 'g--',label =str(dead)+" with intent")
	else:
		plt.plot(cars, avg, 'b--',label =str(dead)+" with intent")
	check += 1
	plt.draw()
# plt.plot(cars, avg, 'b--')
...
plt.title("Intent comparison for Map C")
plt.xlabel("# agents")
plt.ylabel("Graph Idleness")
plt.legend()
# plt.show()
plt.savefig('intent.png', dpi = 100)

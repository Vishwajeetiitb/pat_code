...
class rl_env(object):
    ...
    def __init__(self):
        ...
        self.state=[0.0, 0.0, 0.0, 0.0] #change
        self.nA = 4
        self.nS = 25
        self.reward=[0.0, 0.0, 0.0, 0.0] #change
    ...
    def reset(self, route0, route1, route2, route3): #change
        rou_curr=[[],[], [], []] #change
        rou_curr[0].append(route0)
        rou_curr[1].append(route1)
        rou_curr[2].append(route2)
        rou_curr[3].append(route3)       #change
        self.state=[0, 24, 7, 17]
        ...
        traci.route.add('rou_3', rou_curr[3])
        ...
        traci.vehicle.add(vehID = 'veh3',routeID = 'rou_3', typeID = "car1")
        return self.state
#end of class
...
def run(env):
    ...
    rou_curr2= "7to"+str(random.choice([6,12, 2,12]))
    rou_curr3 = "17to"+str(random.choice([16,18, 12,22]))
    rou_curr=[rou_curr0, rou_curr1, rou_curr2, rou_curr3]
    env.reset(rou_curr0, rou_curr1, rou_curr2, rou_curr3)
    sumo_step=1.0
    cr=[0.0, 0.0, 0.0, 0.0]
    rl_step=1.0
    idle=[np.zeros((25,1)) for _ in range(4)]
    global_idl=np.zeros((25,1))
    global_v_idl=[[] for _ in range(25)]
    v_idle=[[[] for _ in range(25)] for _ in range(4)]
    edge=[0,0,0, 0]
    prev_node=env.state
    curr_node=[0.0,0.0,0.0, 0.0]
    temp_n=[0.,0.,0., 0.]
    temp_p=[0,24,7, 17]
    ga=[]
    ...
    while traci.simulation.getMinExpectedNumber()>0:
        ...
        edge[3]=traci.vehicle.getRoadID('veh3')

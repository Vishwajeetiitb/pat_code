...
path = './data/' + f.read()
os.system('rm -rf '+path+'individual/grids_img')
os.system('mkdir '+path+'individual/grids_img')
for RunId in range(10):
    fig, axs = plt.subplots(size, size)
    plt.setp(axs,ylim=(50,500))
    fig.set_size_inches(19.20, 10.80)
    RunPath = path+'run' + str(RunId) + '_peaks/'
    xls = pd.ExcelFile(RunPath+'run'+str(RunId)+'_peaks.xlsx')
    ...
    # plt.ylim([2,5])
    plt.title('jay bhole')
    plt.savefig(path+'individual/grids_img/run'+ str(RunId)+'grid.png',bbox_inches = 'tight', dpi=100)

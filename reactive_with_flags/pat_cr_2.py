    ...
    while traci.simulation.getMinExpectedNumber()>0:
        ...
        for i in range(2):
            if prev_node[i]!=curr_node[i]:
                ...
                prev_reward=env.reward_out(idle[0], prev_node[i], i)[0]
                print('reward on prev step: ', prev_reward)
                v_idle[0][int(prev_node[i])].append(prev_reward.copy())
                ...
                #     fa[j]= bool_f
                # print(fa)
                action=CR_patrol(idle[0],curr_node[i],env)
